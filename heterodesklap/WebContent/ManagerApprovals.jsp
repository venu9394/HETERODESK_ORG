...
<script>
 ...
 function ICT_Req(data){
	 ...
	 bootbox.prompt({
		    title: " <B style='font-size: 14px;'>Reason for Leave / OD ::</B><br/> <div style='color:#0088cc;font-size: 13px;'>  "+DATA+" <div> <div>&nbsp;</div><div> <B style='font-size: 14px;'>Write Your Comment : </div>" ,
		    inputType: 'textarea',
		     buttons: {
		        confirm: {
		            label: 'Confirm',
		    ...
		    try{
		    	if(result!=null ){
		    	 var dd=AttendanceRequest_Month_VEN(data.alt,data.name,data.value,data.id,result);
		    	 //alert(dd);
		    	// return true;
		    	} 
		    }catch(err){
		        ...
		        //AttendanceRequest_Month_VEN(data.alt,data.name,data.value,data.id,result);  
		  }
 });
	 
	 }catch(err){
		 alert("Invalid request please contact system admin...!");
	 }
 ...
 function AttendanceRequest_Month_VEN(val1,val2,val3,val4,result){
	 ...
	 // alert(val1 + "~"  + val2 + "~" + val3 );
	   ...
	   // alert("New");
   var formData = {id:""+val1+"",rid:""+val2+"",flag:""+val3+"",comment:""+result+""};
	try{
	    $.ajax({
	          type: "post",
	          url: "ReqAcpt_app",
  ...
  function disableBackButton() {
							...
							<div class="col-xs-12 col-sm-6 col-md-12">
								<div class="panel panel-primary">
								   ...
								   <span id='Requ_Date_ERROR' style='color:red;' align='right'> Please Submit your approvals before payroll closing. </span>
									...
									<div class="panel-body panel-body-mgrapr">
										<div class="col-md-12"  >
    ...
    <li><a href="#" data-toggle="tooltip" data-placement="right" title="Hooray!">Right</a></li>
														...
														<!--  <td class="text-center"> <span  id="{{ x.DATE }}_ST" > {{ x.DAREQ }} </span> </td>  -->
														 ...
														 <!--  <td class="text-center" style="word-wrap: break-word;white-space:normal;width:160px !important;"> {{ x.MESSAGE }} {{ x.MESSAGE }} {{ x.MESSAGE }} {{ x.MESSAGE }} </td> -->
														 <td class="text-center"> <span  id="{{ x.RID }}_ST" > {{ x.Manager_Status }}  </span> </td>
														 
														 <td class="text-center" colspan='2'> 
														 <input type='text' id='{{ x.ID }}_{{ x.RID }}' value="{{ x.MESSAGE }}" style='display:none;' />
														 <input type='button' class='btn btn-primary btn-sm'  name='{{ x.RID }}' alt='{{ x.ID }}' id='{{ x.RID }}_1'   onclick='ICT_Req(this);'  value='{{ x.B1T }}' style='display:{{ x.B1 }}' ng-disabled="{{x.BUTTACT}}" /> &nbsp;
														  <input type='button' class='btn btn-primary btn-sm'  alt='{{ x.ID }}' name='{{ x.RID }}'  id='{{ x.RID }}_2' onclick='ICT_Req(this);'   value='{{ x.B2T }}'  style='display:{{ x.B2 }}'  ng-disabled="{{x.BUTTACT}}" /> </td> 
       ...
       <script src="assets/javascripts/toolbox.js"></script>
