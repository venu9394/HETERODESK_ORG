...
public class ReqAcpt_app extends HttpServlet {
   ...
   if(Authcode>0){
	 ...
	 if(auth_apr_flag){
		...
		//Orginal	updateflag.append("UPDATE hclhrm_prod_others.tbl_emp_attn_req set FLAG='"+flagupdate+"',comments= '"+comments+"',approvedby='"+Emp_id+"',approvedname='"+Emp_name+"' , MAIL_STATUS='N' WHERE FLAG in('P','A') AND EMPLOYEEID="+empid+" AND RID="+rid+" ");
			
			updateflag.append("UPDATE hclhrm_prod_others.tbl_emp_attn_req set FLAG='"+flagupdate+"',comments=?,approvedby='"+Emp_id+"',approvedname='"+Emp_name+"' , MAIL_STATUS='N' WHERE FLAG in('P','A') AND EMPLOYEEID="+empid+" AND RID="+rid+" ");
